...
var show_navigation_buttons = {
	...
	activated : function() {
		...
		window.$('<div id="ext_search"></div>').prependTo('body');
		...
		window.$('#ext_search').css('background-image', 'url(data:image/png;base64,'+searchImg+')');
		
		// Create the search event
		window.$('#ext_search').toggle(
	...
	showSearch : function() {
		...
		var clone = window.$('.lapozo:last').next().next().clone().appendTo('body');
	...
	hideSearch : function() {
		window.$('#ext_overlay_search').remove();
	}
};
...
var update_fave_list = {
	...
	activated : function() {
		...
		// Add click event
		window.$('#ext_refresh_faves').click(function() {
	...
	refresh : function() {
		...
		window.$('#ext_refresh_faves img').attr('src', 'data:image/png;base64,'+inProgressImg);
		...
		window.$.ajax({
			...
			success : function(data) {
				...
				// Set 'completed' icon
				window.$('#ext_refresh_faves img').attr('src', 'data:image/png;base64,'+completedImg);
				
				// Set back the normal icon in 1 sec
				setTimeout(function() {
					window.$('#ext_refresh_faves img').attr('src', 'data:image/png;base64,'+refreshImg);
				}, 2000);
...
var make_read_all_faves = {
	...
	activated : function() {
		...
		// Add click event
		window.$('#ext_read_faves').click(function() {
	...
	makeread : function() {
		...
		if(confirm('Biztos olvasottnak jelölöd az összes kedvenced?')) {
			...
			// Set 'in progress' icon
			window.$('#ext_read_faves img').attr('src', 'data:image/png;base64,'+makereadedWaitingImg);
			...
			window.$('.ext_faves').next().find('div a').each(function() {
				...
				if( window.$(this).parent().is('.std0') ) {
					return true;
				}
				
				// Also dont bother readed topics
				if( window.$(this).find('font').length == 0) {
					return true;
				}
			...
			window.$('.ext_faves').next().find('div a').each(function() {
				...
				window.$.get( window.$(this).attr('href'), function() {
					
					window.$(ele).find('font').remove();
					window.$(ele).find('.ext_short_comment_marker').remove();
					
					if(dataStore['fav_show_only_unreaded'] == 'true') {
						window.$(ele).parent().addClass('ext_hidden_fave');
					}
			...
			var interval = setInterval(function() {
				...
				if(count == counter) {
					...
					// Set 'completed' icon
					window.$('#ext_read_faves img').attr('src', 'data:image/png;base64,'+makereadedCompletedImg);
			
					// Set normal icon
					setTimeout(function() {
						window.$('#ext_read_faves img').attr('src', 'data:image/png;base64,'+makereadedImg);
					}, 2000);
					
					if(dataStore['fav_show_only_unreaded'] == 'true') {
					
						// Add 'no unreaded topics' message
						if( window.$('#ext_filtered_faves_error').length == 0) {
							window.$('.ext_faves').next().find('div:last').after('<p id="ext_filtered_faves_error">Nincs olvasatlan téma</p>');
						}
						
						// Hide all remaining topics
						if(dataStore['fav_show_only_unreaded'] == 'true') {
							window.$('.ext_faves').next().find('div').addClass('ext_hidden_fave');
						}
					}
...
var overlay_reply_to = {
	...
	activated : function() {
		...
		window.$('textarea:first').attr('tabindex', '3');
		window.$('textarea:first').closest('div').find('a:last').attr('tabindex', '4');

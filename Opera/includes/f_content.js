...
var show_navigation_buttons = {
	...
	activated : function() {
		...
		$('<div id="ext_scrolltop">&#9650;</div>').prependTo('body').css('display', 'none');	
		...
		$('<div id="ext_back">&#9664;</div>').prependTo('body').css('display', 'none');	
		...
		if(!document.location.href.match('cikkek') && !document.location.href.match('listazas_msg.php')) {
			...
			$('<div id="ext_whitelist" title="'+title+'">'+status+'</div>').prependTo('body').css('display', 'none');
		...
		if(isLoggedIn() || document.location.href.match('listazas_msg.php')) {
			...
			// Create faves button
			$('<div id="ext_nav_faves"></div>').prependTo('body').css('display', 'none');
	...
	activated : function() {
		...
		comments.each(function() {
			...
			if($(this).find('.ext_comments_for_me_indicator').length == 0) {
				...
				$(this).css('position', 'relative').append('<img src="data:image/png;base64,'+img+'" class="ext_comments_for_me_indicator" style="display: none;">');
...
if (window.top === window) {
	$(document).ready(function() {
    	...
    	opera.extension.postMessage({ name : 'LoadInjectedCSS', message : 'css/restore.css' });
